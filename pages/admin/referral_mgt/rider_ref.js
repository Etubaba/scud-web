...
const Rider = ({ users, states }) => {
  ...
  const rows = riderList
    ...
    .map((element) => (
      <tr
        ...
        className=" text-center hover:translate-x-1 cursor-pointer transition-all  hover:shadow-md hover:border text-sm  text-textColor border-b"
        key={element.id}
      >
  ...
  useEffect(() => {
    ...
    const filteredRider = riders.filter(
      (el) => el.state?.name?.toLowerCase() == location.toLowerCase()
    );
    setRiderList(filteredRider);
  }, [location]);

  return (
    <div>
      {" "}
      <p className="text-base tracking-wide  font-semibold">Rider's Referral</p>
      {riders.length > 0 && (
      ...
      {riders.length > 0 ? (
              ...
              <tr className="border-b ">
                ...
                <td className="">
                  <div className="flex md:text-base text-xs justify-center">
                    ...
                    <p className="md:text-base  text-xs">Riders</p>
                  </div>
                </td>
        ...
        <EmptyTable Icon={BsPeople} name={"referrers"} title={"No Rider With Referrees"} />
      )}
      {/* table end here  */}
      {/* <Pagination /> */}
    </div>
