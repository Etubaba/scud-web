...
import { BsBarChart, BsCheck2 } from "react-icons/bs";
...
const City = ({ cities }) => {
  ...
  const [stateList, setStateList] = useState(cities.data);
  ...
  const activateCity = async () => {
    ...
    try {
      ...
      const { data } = await axios.patch(`${BASE_URL}cities/${state?.id}`, {
      ...
      >
        ...
        <td className="md:text-base text-xs p-3 ">
          <span className="flex space-x-3 justify-center">
            {/* <Switch
              // onChange={(e) => activateCity(e, element.id, element.is_active)}
              onChange={(e) => activateCity(e, idx, element.id)}
              value={active}
            /> */}
  ...
  const totalPages = Math.ceil(cities?.total_records / cities?.page_size);

  const componentToPrintRef = useRef();

  const handlePrintDoc = useReactToPrint({
    content: () => componentToPrintRef.current,
    documentTitle: "cities",
    onAfterPrint: () => null
  });
  ...
  return (
    <div>
      ...
      <div className="flex flex-row mb-7 md:mb-5 justify-between items-center">
        <p className="md:text-lg text-sm tracking-wide font-semibold">Manage Cities</p>
      </div>
      {cities.data.length > 0 && (
        <div className="flex flex-col-reverse md:flex-row md:justify-between">
          <PrintTable handlePrintDoc={handlePrintDoc} table_id={"#cities_"} file_name={"Cities"} />
      ...
      {cities.data.length === 0 ? (
        <div className="mt-4">
          <EmptyTable Icon={MdLocationCity} title="No Cities " name="citiess" />
        </div>
      ) : (
        <div
          id="cities_"
      ...
      {/* table end here  */}
      <Pagination isAdmin={true} serverData={cities} setData={setStateList} />
        ...
        <div className="w-[18rem] md:w-[24rem]  h-auto">
          <div className="flex flex-col space-y-3 justify-center items-center">
            ...
            <p className="text-lg font-semibold mt-2">
              {state?.is_active ? "Deactivate" : "Active"} City
            </p>
            <p className="text-sm text-textColor mt-2">
              You are about to {state?.is_active ? "deactivate" : "active"} this City.
            </p>
          </div>
...
export async function getServerSideProps(context) {
  ...
  const res = await fetch(`${BASE_URL}cities/paginated?state_id=${id}`, {
    headers: {
  ...
  if (cities?.statusCode !== undefined && cities?.statusCode === 401) {
    try {
