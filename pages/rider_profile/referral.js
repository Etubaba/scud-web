...
import { BsCheckCircle, BsClock, BsPeople, BsPersonCircle, BsShare } from "react-icons/bs";
...
const Referral = ({ referrals }) => {
  ...
  return (
    <div className="md:p-5 p-2">
      {" "}
      <div className="md:flex mb-8 md:mb-10 md:justify-between items-center">
        <div>
          <p className="font-semibold  text-base md:text-lg tracking-wider">Referral</p>
          <p className="md:text-sm text-xs mt-2  text-textColor">Your list of referrals</p>
        </div>
        ...
        <span className="flex flex-col md:flex-row space-y-2 md:space-y-0 mt-5 md:-mt-5 md:space-x-2">
          ...
          <button
            ...
            className="bg-scudGreen hover:to-blue-500 text-[10px] text-white rounded-md px-1 py-[2px]"
          >
            Copy
          </button>
        </span>
      </div>
      <div className="w-full flex flex-col md:flex-row mb-8 space-y-4 md:space-y-0 md:space-x-4 md:my-10">
        <DashboardCompo
          ...
          Icon={BsPeople}
        />
        <DashboardCompo value={8} title="Active" color="green" Icon={BsPeople} />
        <DashboardCompo
          value={"₦450"}
      ...
      <p className="text-[#1E202B] text-sm mb-5 font-semibold">My Referral List</p>
      {referrals.length === 0 ? (
      ...
      {/* table end here  */}
      <Modal onClose={() => setCopied(false)} open={copied}>
