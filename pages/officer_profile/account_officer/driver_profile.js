...
const Driver_profile = ({ user, timeOnline }) => {
  ...
  const rows = referrees.slice(0, 3).map((element) => (
    <tr
      // onClick={() => {
    ...
    >
      ...
      <td className="flex justify-center py-2 items-center space-x-2">
        <img src={element.referree.avatar} className="rounded-full h-7 w-7 md:h-8 md:w-8" alt="" />
        <p className=" text-xs ">{element.referree.name}</p>
      </td>

      <td className="md:text-xs text-xs p-3">{element.usertype}</td>
      <td className="md:text-xs text-xs p-3">{element.sex}</td>
  ...
  return (
    <div>
        ...
        <div className="bg-white justify-center  flex p-8 border w-full md:w-1/4 h-auto rounded-md">
          <div className="flex flex-col  items-center">
            ...
            <div className="flex my-3 space-x-2">
              ...
              >
                <p className="text-center  text-white tracking-wider text-xs">{status}</p>
              </div>
            </div>

            <Rating size={"xs"} rating={sumArray(trips)} readOnly={true} />
          ...
          <div className="flex space-x-20 ">
            <div className="flex flex-col ">
              <span className="mb-5 flex space-y-2 flex-col">
                ...
                <p className="md:text-sm text-xs text-textColor">{bank_account?.bank_name}</p>
              </span>
              <span className=" flex space-y-2 flex-col">
      ...
      <div className="flex justify-between">
        <p className="font-[400]">
          List of Referees<b className="text-textColor/50"> (34)</b>
        </p>
      ...
      <div className="bg-white p-5 flex-col md:flex-row space-y-6 md:space-y-0 md:space-x-8 justify-between rounded-md flex border">
        <div className="bg-adminbg w-full md:w-1/3 rounded-md p-4">
          <span className="flex  space-x-2">
            {" "}
            <p className="text-2xl font-bold">{sumArray(user.reviews_recieved)}.0</p>
